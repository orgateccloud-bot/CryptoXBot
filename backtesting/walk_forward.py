...
def walk_forward(symbol="BTCUSDT", intervalo="1h", janela_treino=500,
                 janela_teste=100, capital_inicial=1000.0):
    """
    ...
    if len(k1h) < janela_treino + janela_teste + 100:
        return {"erro": f"Dados insuficientes ({len(k1h)} candles). Precisa de {janela_treino + janela_teste + 100}."}
    ...
    while current + janela_teste < fim_dados:
        ...
        for i in range(teste_inicio, teste_fim):
            ...
            if posicao:
                ...
                if mn <= posicao["stop"]:
                    ...
                    todas_ops.append({"resultado": pnl, "resultado_pct": round(pnl_pct, 2),
                                       "tipo_saida": "STOP", "janela": janela_num,
                                       "entrada_dt": posicao.get("dt", ""),
                                       "saida_dt": datetime.fromtimestamp(ts1h[i]/1000).strftime("%d/%m/%Y %H:%M"),
                                       "preco_entrada": posicao["entrada"], "preco_saida": round(ps, 2)})
                    ops_janela += 1
                    posicao = None
                    ...
                    todas_ops.append({"resultado": pnl, "resultado_pct": round(pnl_pct, 2),
                                       "tipo_saida": "TARGET", "janela": janela_num,
                                       "entrada_dt": posicao.get("dt", ""),
                                       "saida_dt": datetime.fromtimestamp(ts1h[i]/1000).strftime("%d/%m/%Y %H:%M"),
                                       "preco_entrada": posicao["entrada"], "preco_saida": round(pt, 2)})
                    ops_janela += 1
                    ganhos_janela += 1
                    posicao = None
            ...
            if posicao is None:
                ...
                score, decisao, fator, _ = _score_backtest(
                    preco, e20, e50, rsi_v, atr_v, atr_med, vr, bw_v,
                    sum(x for x in bw[max(0,i-20):i] if x) / max(1, len([x for x in bw[max(0,i-20):i] if x])),
        ...
        dt_fim = datetime.fromtimestamp(ts1h[min(teste_fim, len(ts1h)-1)]/1000).strftime("%d/%m/%Y")
        ...
        print(f"  Janela {janela_num:2d}: {dt_ini}-{dt_fim} | "
...
def imprimir_relatorio(r):
    ...
    print(f"{'='*65}")
    print(f"  Janelas:  {r['total_janelas']} (treino: {r['janela_treino']} / teste: {r['janela_teste']})")
    ...
    print(f"  {'-'*60}")
    for j in r["janelas"]:
        print(f"  {j['janela']:3d} {j['periodo']:>25} {j['treino_size']:6d} "
