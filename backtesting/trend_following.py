...
def rodar_pesquisa(intervalo="4h", holdout=False) -> dict:
    ...
    return {
        "por_ativo": por_ativo,
        "pool": _metricas_pool(trades_pool, por_ativo),
        "pnl_por_ano": pnl_por_ano,
        "pnl_por_ativo": pnl_por_ativo,
        "intervalo": intervalo,
        "holdout": holdout,
    }
...
def _metricas_pool(rets_pct, por_ativo) -> dict:
    ...
    ret_med = np.mean(
        [por_ativo[s]["retorno_total_pct"] for s in por_ativo if por_ativo[s]["n_trades"]]
    )
    dd_med = np.mean(
        [por_ativo[s]["max_drawdown_pct"] for s in por_ativo if por_ativo[s]["n_trades"]]
    )
...
def curva_carteira(intervalo="1d", holdout=False, capital_por_ativo=1000.0) -> dict:
    ...
    return {
        "n_trades": len(eventos),
        "n_ativos": n_ativos,
        "capital_inicial": capital_inicial,
        "capital_final": cap,
        "ret_total_pct": ret_total,
        "anos": anos,
        "anualizado_pct": anualizado,
        "max_dd_pct": max_dd,
        ...
        "frac_anos_pos": frac_anos_pos,
        "anos_pnl": dict(sorted(anos_ok.items())),
        "por_ativo": por_ativo,
    }
...
def imprimir_carteira(r, holdout=False):
    ...
    print(f"    Retorno total:  {r['ret_total_pct']:+.2f}%  " f"({r['anualizado_pct']:+.2f}% a.a.)")
    print(f"    Max drawdown:   {r['max_dd_pct']:.2f}%   |   MAR: {r['mar']:.2f}")
    print(
        f"    B&H equal-weight no período: {r['bh_ret_medio_pct']:+.2f}% "
        f"(DD {r['bh_dd_medio_pct']:.1f}%)"
    )
    print(
        f"    -> DD da carteira e {r['max_dd_pct']/r['bh_dd_medio_pct']*100:.1f}% do DD do B&H"
        if r["bh_dd_medio_pct"] > 0
        else ""
    )
    if r["anos_pnl"]:
        print("\n    PnL por ano:")
    ...
    print(
        f"    [{'x' if c5 else ' '}] >= 60% dos anos positivos     ({r['frac_anos_pos']*100:.0f}%)"
    )
...
def analise_por_regime(intervalo="1d", holdout=False) -> dict:
    ...
    for a in sorted(bh_por_ano):
        ...
        linhas.append(
            {"ano": a, "regime": regime, "bh_pct": bh, "strat_pct": st, "captura": captura}
        )
...
def imprimir_regime(reg):
    ...
    for L in linhas:
        ...
        print(
            f"  {L['ano']:>5} {L['regime']:<8} {L['bh_pct']:>+10.1f} "
    ...
    if caps:
        ...
        print(
            f"    [{'x' if ok_bull else ' '}] captura média no bull >= 30%  "
...
def imprimir(res):
    ...
    if not p["n_trades"]:
        ...
        return
    print(
        f"    n={p['n_trades']} | win {p['win_rate']:.1f}% | PF {p['profit_factor']:.2f} | "
        f"payoff {p['payoff_ratio']:.2f} | exp {p['expectancy_net_pct']:+.3f}%/trade"
    )
    print(
        f"    retorno médio {p['ret_medio_pct']:+.2f}% (DD {p['dd_medio_pct']:.1f}%) vs "
    ...
    if rodada2:
        print(
            f"    [{'!' if frag else 'x'}] robustez de regime            "
...
def main():
    ...
    ap.add_argument(
        "--carteira", action="store_true", help="objetivo risco-controlado: curva/DD de CARTEIRA"
    )
    ap.add_argument("--holdout", action="store_true", help="USO UNICO — so apos a pesquisa aprovar")
    ap.add_argument(
        "--regime",
        action="store_true",
