...
def rodar(intervalo_entrada="1h", intervalo_mtf="4h", capital_inicial=1000.0):
    ...
    for i in range(55, len(k1h)):
        ...
        if posicao:
            ...
            if mn <= posicao["stop"]:
                ...
                capital += pnl
                operacoes.append({"resultado": pnl, "resultado_pct": round((ps-posicao["entrada"])/posicao["entrada"]*100,2),
                                   "tipo_saida": "STOP", "saida_dt": datetime.fromtimestamp(ts1h[i]/1000).strftime("%d/%m/%Y %H:%M"),
                                   "entrada_dt": posicao.get("dt",""), "preco_entrada": posicao["entrada"], "preco_saida": round(ps,2)})
                posicao = None; saiu = True
                ...
                capital += pnl
                operacoes.append({"resultado": pnl, "resultado_pct": round((pt-posicao["entrada"])/posicao["entrada"]*100,2),
                                   "tipo_saida": "TARGET", "saida_dt": datetime.fromtimestamp(ts1h[i]/1000).strftime("%d/%m/%Y %H:%M"),
                                   "entrada_dt": posicao.get("dt",""), "preco_entrada": posicao["entrada"], "preco_saida": round(pt,2)})
                posicao = None; saiu = True
        ...
        if posicao is None:
            ...
            if ok_ema:  filtros_contagem["ema"]     += 1
            if ok_rsi:  filtros_contagem["rsi"]     += 1
            if ok_atr:  filtros_contagem["atr"]     += 1
            if ok_vol:  filtros_contagem["volume"]  += 1
            if ok_vwap: filtros_contagem["vwap"]    += 1
            if ok_mtf:  filtros_contagem["mtf"]     += 1
            if ok_bb:   filtros_contagem["bollinger"] += 1
    ...
    # Max Drawdown
    pico = capital_inicial; cap = capital_inicial; max_dd = 0
    for o in operacoes:
        cap += o["resultado"]
