...
- **EMA/RSI reimplementados** em `dashboard.py`, `analise_mercado.py` — drift de cálculo vs `indicadores.py` (a versão numpy canônica), mas ambos são cosméticos/mortos no caminho ao vivo (P1-5 confirmou: `backtesting/motor.py`'s cópia já era um wrapper fino de `indicadores.py`, não uma reimplementação — baixo risco, não precisa de ação). `analise_mercado.py` em si parece código órfão (script standalone, mercado **futures**, não importado por `main.py`) — candidato a aposentar (@Zeta) em vez de consolidar.
