...
class TestGetPrecoAtual:
    def _payload(self, preco="64000.0", var="1.5", maxi="65000.0", mini="63000.0",
                 vol="500.0", quoteVol="32000000000.0"):
        return {
            "lastPrice": preco,
    ...
    def test_retorna_variacao(self):
        with patch("analise_mercado.requests.get", return_value=_mock_resp(self._payload(var="2.3"))):
            r = analise_mercado.get_preco_atual()
    ...
    def test_todos_campos_presentes(self):
        with patch("analise_mercado.requests.get", return_value=_mock_resp(self._payload())):
            r = analise_mercado.get_preco_atual()
        for campo in ("preco", "variacao_24h_%", "maximo_24h", "minimo_24h", "volume_24h_btc", "volume_24h_usdt"):
...
class TestGetOrderBook:
    ...
    def test_campos_retornados(self):
        with patch("analise_mercado.requests.get", return_value=_mock_resp(self._ob_payload())):
            r = analise_mercado.get_order_book()
        for campo in ("maior_suporte_preco", "maior_resistencia_preco", "liquidez_compra_usdt", "liquidez_venda_usdt", "pressao_dominante"):
            assert campo in r
...
class TestGetMediasMoveis:
    def _klines(self, preco_base=64000.0, n=60):
        ...
        return [
            [0, str(preco_base + i * 10), str(preco_base + i * 10 + 100),
             str(preco_base + i * 10 - 50), str(preco_base + i * 10), "500"]
            for i in range(n)
        ]
    ...
    def test_tendencia_baixa_detectada(self):
        ...
        klines = [
            [0, str(65000.0 - i * 10), str(65000.0 - i * 10 + 50),
             str(65000.0 - i * 10 - 100), str(65000.0 - i * 10), "500"]
            for i in range(60)
        ]
