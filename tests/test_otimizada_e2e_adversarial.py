...
def test_contrato_estendido_chaves_telemetria(patch_otimizada):
    ...
    for chave in CHAVES_CONTRATO:
        assert chave in r
    for extra in ("bw", "bw_media", "atr", "atr_media", "vwap",
                  "tend_4h", "regime", "volume_rel", "symbol",
                  "score_result", "tamanho_fator"):
    ...
    assert r["volume_rel"] == pytest.approx(round(2000.0 / media_vol, 2),
                                            abs=0.01)
...
def test_simbolo_desconhecido_cai_em_btc_sem_quebrar(patch_otimizada):
    ...
    r = otimizada.analisar("DOGEUSDT", cvd_atual=10.0, ml_prob=0.80,
                           ensemble_result=_ensemble(prob=0.85))
...
def test_par_eth_usa_stop_target_proprios(patch_otimizada):
    ...
    r = otimizada.analisar("ETHUSDT", cvd_atual=10.0, ml_prob=0.80,
                           ensemble_result=_ensemble(prob=0.85))
...
def test_operar_reduzido_define_tamanho_fator_meio(patch_otimizada):
    ...
    """
    d1h = _serie_klines(n=100, base=100.0, passo=0.2, vol=1000.0,
                        vol_ultima=1100.0)
    ...
    r = otimizada.analisar("BTCUSDT", cvd_atual=1.0, ml_prob=0.55,
                           ensemble_result=_ensemble(prob=0.55))
...
def test_cvd_none_nao_bloqueia_compra(patch_otimizada):
    ...
    r = otimizada.analisar("BTCUSDT", cvd_atual=None, ml_prob=0.80,
                           ensemble_result=_ensemble(prob=0.85))
...
def test_cvd_negativo_bloqueia_compra_vira_aguardar(patch_otimizada):
    ...
    d1h = _serie_klines(n=100, base=100.0, passo=1.0, vol_ultima=2000.0)
    d4h = _serie_klines(n=60, base=100.0, passo=1.0)
    chamadas = patch_otimizada(d1h, d4h, _regime("TENDENCIA_ALTA", score=90),
                               _fear(valor=50), _suporte())

    r = otimizada.analisar("BTCUSDT", cvd_atual=-5.0, ml_prob=0.80,
                           ensemble_result=_ensemble(prob=0.85))
...
def test_ensemble_none_e_prever_lanca_usa_fallback(patch_otimizada,
                                                   monkeypatch):
...
def test_ensemble_sem_prob_usa_ml_prob_como_fallback(patch_otimizada):
    ...
    r = otimizada.analisar("BTCUSDT", cvd_atual=10.0, ml_prob=0.77,
                           ensemble_result=ensemble_sem_prob)
...
def test_imprimir_ramo_ml_prob_simples(patch_otimizada, capsys):
    ...
    ensemble_simples = {"prob_ensemble": 0.80, "pode_operar": True,
                        "confianca": "MEDIA"}
    r = otimizada.imprimir("BTCUSDT", cvd_atual=10.0, ml_prob=0.80,
                           ensemble_result=ensemble_simples)
...
def test_imprimir_venda_mostra_stop_target(patch_otimizada, capsys):
    ...
    patch_otimizada(d1h, d4h, _regime("TENDENCIA_BAIXA", score=95),
                    _fear(valor=50), _suporte())
