...
class TestScorePesos(unittest.TestCase):
    ...
    def test_peso_ml_igual_20(self):
        self.assertEqual(self.PESOS["ml"], 20,
            "Peso do ML deve ser 20% após melhoria")

    def test_soma_pesos_igual_100(self):
        total = sum(self.PESOS.values())
        self.assertEqual(total, 100,
            f"Pesos devem somar 100, soma atual: {total}")

    def test_regime_e_maior_ou_igual_15(self):
        self.assertGreaterEqual(self.PESOS["regime"], 15,
            "Regime deve manter peso relevante (>= 15%)")

    def test_ml_e_maior_componente_ou_empatado(self):
        maior = max(self.PESOS.values())
        self.assertGreaterEqual(self.PESOS["ml"], maior * 0.95,
            "ML deve ser o maior componente ou empatado com o maior")
...
class TestFSRS(unittest.TestCase):
    ...
    def test_fator_neutro_sem_historico(self):
        features = {"regime": "TENDENCIA_ALTA", "rsi": 58, "dist_ema20": 0.004,
                    "cvd_score": 70, "vol_rel": 1.5}
    ...
    def test_quantizar_features_retorna_string(self):
        features = {"regime": "LATERAL", "rsi": 42, "dist_ema20": -0.001,
                    "cvd_score": 40, "vol_rel": 0.8}
    ...
    def test_fator_aumenta_apos_lucros(self):
        features = {"regime": "TENDENCIA_ALTA", "rsi": 60, "dist_ema20": 0.005,
    ...
    def test_fator_diminui_apos_perdas(self):
        features = {"regime": "LATERAL", "rsi": 50, "dist_ema20": 0.001,
    ...
    def test_padroes_diferentes_sao_independentes(self):
        feat_alta  = {"regime": "TENDENCIA_ALTA", "rsi": 60, "dist_ema20": 0.005,
                      "cvd_score": 75, "vol_rel": 1.8}
        feat_lateral = {"regime": "LATERAL", "rsi": 50, "dist_ema20": 0.0,
                        "cvd_score": 50, "vol_rel": 0.9}
        ...
        self.assertGreater(fator_alta, fator_lateral,
    ...
    def test_avaliar_com_detalhe_sem_historico(self):
        features = {"regime": "INDEF", "rsi": 50, "dist_ema20": 0.0,
                    "cvd_score": 50, "vol_rel": 1.0}
    ...
    def test_avaliar_com_detalhe_apos_trades(self):
        features = {"regime": "TENDENCIA_ALTA", "rsi": 62, "dist_ema20": 0.006,
                    "cvd_score": 78, "vol_rel": 1.9}
        for _ in range(6):
            self.fsrs.registrar_resultado(features, lucro_pct=0.02)
...
class TestEnsembleFSRS(unittest.TestCase):
    ...
    def test_fsrs_disponivel_no_ensemble(self):
        import ensemble
        self.assertTrue(ensemble._FSRS_DISPONIVEL,
            "_FSRS_DISPONIVEL deve ser True (fsrs_trading.py presente)")

    def test_prever_retorna_fator_fsrs(self):
        """prever() deve retornar chave fator_fsrs."""
        import ensemble
        result = ensemble.prever()
        self.assertIn("fator_fsrs", result,
            "Resultado do ensemble deve conter 'fator_fsrs'")
    ...
    def test_fator_fsrs_neutro_sem_features(self):
        ...
        self.assertEqual(result["fator_fsrs"], 0.5,
            "Sem features_fsrs, fator FSRS deve ser 0.5")
    ...
    def test_estrutura_resultado_ensemble(self):
        ...
        result = ensemble.prever()
        campos = [
            "prob_ensemble", "prob_xgb", "prob_lstm", "concordancia",
            "confianca", "pode_operar", "motivo", "regime",
            "pesos_aplicados", "fator_fsrs", "fsrs_detalhe",
        ]
        for campo in campos:
    ...
    def test_pesos_aplicados_somam_um(self):
        ...
        self.assertEqual(soma, 1.0,
            f"Pesos XGB+LSTM devem somar 1.0, soma: {soma}")
...
class TestOllamaCliente(unittest.TestCase):
    ...
    def test_modelos_configurados_corretos(self):
        ...
        self.assertEqual(MODELO_RAPIDO, "gemma3:4b",
            "Modelo rápido deve ser gemma3:4b (3.2GB, cabe na VRAM)")
        self.assertEqual(MODELO_ANALITICO, "llama3:latest",
            "Modelo analítico deve ser llama3:latest (4.4GB)")
    ...
    def test_fallback_analisar_mercado_sem_ollama(self):
        ...
        resultado = c.analisar_mercado(
            regime="TENDENCIA_ALTA", score=72, prob_ml=0.67, preco=85000
        )
    ...
    def test_fallback_explicar_sinal_sem_ollama(self):
        ...
        resultado = c.explicar_sinal(
            sinal="COMPRA", score=74,
            componentes={"ml": 18, "regime": 16},
            prob_ml=0.68, regime="TENDENCIA_ALTA"
        )
        self.assertIsInstance(resultado, str)
...
class TestOllamaIntegrado(unittest.TestCase):
    ...
    def test_analisar_mercado_retorna_texto(self):
        resultado = self.cliente.analisar_mercado(
            regime="TENDENCIA_ALTA", score=74, prob_ml=0.67,
            preco=85000, rsi=58
        )
        self.assertIsInstance(resultado, str)
    ...
    def test_latencia_gemma3_4b(self):
        ...
        self.assertLess(latencia, 15.0,
            f"Latência {latencia:.1f}s acima do esperado (<15s) para gemma3:4b")
...
class TestRetreinamentoAutomatico(unittest.TestCase):
    ...
    def test_constantes_retreinamento(self):
        import main
        self.assertEqual(main._RETREINAMENTO_DIA, 6,
            "_RETREINAMENTO_DIA deve ser 6 (domingo)")
        self.assertEqual(main._RETREINAMENTO_HORA, 2,
            "_RETREINAMENTO_HORA deve ser 2 (02:00)")
    ...
    def test_iniciar_retreinamento_cria_thread(self):
        ...
        self.assertIn("retrain-weekly", novas,
            "Thread 'retrain-weekly' deve ser criada")
...
if __name__ == "__main__":
    ...
    # Sempre executar
    for cls in [TestScorePesos, TestFSRS, TestEnsembleFSRS,
    ...
    print(f"\n{'='*50}")
    print(f"  {result.testsRun} testes | "
